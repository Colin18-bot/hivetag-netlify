...
exports.handler = async function (event, context) {
  try {
    ...
    if (!weather) return { statusCode: 500, body: "Weather lookup failed" };
    ...
    const dataRows = (rows.length > 1) ? rows.slice(1).filter(r => r.length >= headers.length) : [];
    ...
    if (dataRows.length === 0) {
      ...
      return {
        statusCode: 302,
        headers: { Location: registrationFormUrl },
      };
    }
    ...
    for (const row of dataRows) {
      ...
      const d = dist(parseFloat(lat), parseFloat(lon), rowLat, rowLon);
      if (d < closestDist) {
        closestDist = d;
        closest = row;
      }
    ...
    const inspectionForm = `https://docs.google.com/forms/d/e/1FAIpQLSdVdBrqwRRiPI0phriZLS1eWyaEIIk96wGBemvmvjF7NfMqYg/viewform?usp=pp_url&entry.432611212=${encodeURIComponent(hiveId)}&entry.275862362=${encodeURIComponent(apiary)}&entry.2060880531=${encodeURIComponent(weather)}`;

    return {
      statusCode: 302,
      headers: { Location: inspectionForm },
    };
